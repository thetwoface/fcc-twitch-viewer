...
export default class StreamList extends Component {
	constructor(props) {
		...
		this.state = {
			streams: [
				...
				{id: 'OgamingSC2', data: null},
				{id: 'ESL_SC2', data: null},
				{id: 'noobs2ninjas', data: null},
				{id: 'not-a-valid-account', data: null}
			]
		};
	}
	...
	render() {
		const streamListItems = this.state.streams.map((stream) => {
			return <Stream details={stream.data} key={stream.id}/>;
		});
	...
	componentDidMount() {
		...
		this.state.streams.forEach(stream => {
			let res = this._fetchStreamData(stream.id);
			updatedStreams.push({
				id: stream.id,
				data: res
			});
	...
	_fetchStreamData(streamId) {
		//const query = `https://wind-bow.gomix.me/twitch-api/streams/${streamId}`;
		//const opts = {
		//	method: 'GET',
		//	mode:'no-cors',
		//	headers: new Headers({
		//		'Api-User-Agent': 'fcc-twitch-viewer/1.0 (https://github.com/thetwoface/fcc-twitch-viewer/)'
		//	})
		//};
		//
		//fetch(query, opts)
